...
import 'invitation_card_screen.dart';
import 'card_designs_screen.dart';
...
class _HostDashboardState extends State<HostDashboard> {
  ...
  Widget _buildStatsRow(String eventId) {
    return FutureBuilder<List<dynamic>>(
      future: Future.wait([
        FirebaseService.getTotalGifts(eventId),
        FirebaseService.getConfirmedCount(eventId),
        FirebaseService.giftsStream(eventId).first,
      ]),
      builder: (context, snap) {
        final total = (snap.data?[0] as double?) ?? 0;
        final confirmed = (snap.data?[1] as int?) ?? 0;
        final giftsCount = (snap.data?[2] as List?)?.length ?? 0;
        return Row(
          children: [
            _statCard('💰', '${total.toStringAsFixed(0)} ر.س',
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 12),
          // زر بطاقة الدعوة الجديد
          SizedBox(
            width: double.infinity,
            child: ElevatedButton.icon(
              icon: const Icon(Icons.photo_library),
              label: const Text('بطاقة الدعوة (واتساب)'),
              ...
              onPressed: () {
                Navigator.push(context, MaterialPageRoute(
                  builder: (_) => InvitationCardScreen(event: event),
                ));
              },
            ),
          ),
  ...
  Widget _buildQuickActions(BuildContext context, event) {
    return Column(
      ...
      children: [
        ...
        Row(
          children: [
            _actionCard(context, '🎨', 'التصاميم',
              'تغيير شكل البطاقة',
              () => Navigator.push(context, MaterialPageRoute(
                builder: (_) => const CardDesignsScreen(),
              )).then((_) => _refresh()),
            ),
            const SizedBox(width: 12),
            Expanded(child: const SizedBox()), // فارغ للحفاظ على الترتيب
          ],
        ),
      ],
