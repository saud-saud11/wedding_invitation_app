...
class GuestListScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final guests = MockDataService.getGuestsForEvent(eventId);
    final confirmed = guests.where((g) => g.confirmed && !g.declined).toList();
    final declined = guests.where((g) => g.declined).toList();
    final pending = guests.where((g) => !g.confirmed && !g.declined).toList();
                  ...
                  CircleAvatar(
                    ...
                    child: Text(g.guestName[0],
                      style: GoogleFonts.cairo(
                        ...
                        color: g.confirmed ? AppTheme.success
                            : g.declined ? Colors.red.shade400
                            : AppTheme.brown,
                        fontSize: 18,
                      ),
                    ),
